...
PlanMaster.exec() ─── asyncio.gather() → all plans run in parallel
  ...
  ├── PlanPostEmbeddingWithSummaryForReply → embedding + summarization for replies
...
| `plans/disable_rules.py` | `DisableTaskRule` — pluggable kill-switch mechanism (lives in `plans/`, not `tasks/`) |
...
| `task_started_at` | `float` | `time.perf_counter()` timestamp |
| `task_finished_at` | `float` | `time.perf_counter()` timestamp |
...
- `SPAM_COMMENT`, `BANGER_INITIAL_SCREEN`, `POST_EMBEDDING_WITH_SUMMARY`, `POST_EMBEDDING_WITH_SUMMARY_FOR_REPLY`, `MM_EMB_V4`, `MM_EMB_V5`, `MM_EMB_V5_FOR_REPLY`, `REPLY_RANKING`, `SAFETY_PTOS`, `POST_SAFETY`

### `TaskContext`

Mutable context passed through plan tasks. Accumulates:
- `eligibilities`, `content_categories`, `summary`, `multimodal_post_embedding`, `multimodal_post_embedding_dict`, `reply_ranking_results`, `safety_annotations`, `errors`
