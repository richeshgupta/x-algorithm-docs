...
| Field | Type | Notes |
|-------|------|-------|
| `tweet_id` | `i64` | Primary key (signed) |
...
| `in_network` | `Option<bool>` | `None` defaults to `false` (out-of-network) via `unwrap_or(false)` |
...
Important helpers on `PostCandidate` (from `CandidateHelpers` trait):
- `get_screen_names()` — collects author + retweeted author screen names as `HashMap<u64, String>`.
- `as_tweet_info(is_followed_by_viewer)` — builds the proto `TweetInfo`.

> **Note**: The Phoenix scorer uses `retweeted_tweet_id` as the prediction key (if set), falling back to `tweet_id`. This logic is in `phoenix_scorer.rs`, not a method on the struct.
...
| `user_action_sequence` | User engagement history (for ranking and retrieval) |
...
| `bloom_filter_entries` | Probabilistic impression filter |
| `in_network_only` | True for Ranked Following tab |
| `is_bottom_request` | True for pagination/scroll requests |
| `request_id` | Unique request identifier |

> **Note**: The full `ScoredPostsQuery` struct is large and evolves quickly. Additional fields (topic IDs, cached posts flag, Grok topic arrays, starter packs, etc.) are referenced throughout the codebase — consult `candidate_pipeline/query.rs` directly for the authoritative field list.
...
|---------|-----------|
| `for_you` | Default |
| `topics` | `!query.topic_ids.is_empty()` |
...
`following_user_ids` on the **Thunder service side** can be fetched from Strato when the list is empty AND `debug=true` (`thunder_service.rs:197`). However, home-mixer's `ThunderSource` hardcodes `debug: false` in every request — the Strato path is therefore never triggered from the serving path.
...
- **`in_network == None` defaults to out-of-network** — `scored_posts_server.rs` uses `in_network.unwrap_or(false)`, so a missing value is treated as `false` (out-of-network), not in-network.

- **Retweets are scored on the source tweet** — the Phoenix scorer uses `retweeted_tweet_id` as the prediction key (falling back to `tweet_id`). All reposts of the same content share the same ML prediction.
