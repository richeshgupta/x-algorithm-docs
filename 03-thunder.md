...
3. On **delete**: Inserts `tweet_id` into `deleted_posts` map. Also appends a `TinyPost` entry to `original_posts_by_user` under the key `DELETE_EVENT_KEY` (not the author's user ID) — used to track delete event timestamps for retroactive trim ordering.
...
- `has_video == true`, OR
- It's a retweet whose source post has video AND the source post is **not itself a reply** (`!source_post.is_reply`)

AND the retweet itself is not a reply.
...
- **`DELETE_EVENT_KEY` is the map key, not the author ID** — `original_posts_by_user.entry(DELETE_EVENT_KEY)` stores delete event timestamps under a special sentinel constant as the map key — not under the author's user ID. This groups all delete timestamps in one VecDeque for `finalize_init()` to process retroactively.
