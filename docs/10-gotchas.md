...
### `in_network == None` treated as in-network
**Component**: home-mixer / `OONScorer`, `RankingScorer`  
**Behavior**: If `in_network` is `None`, OON penalty does NOT apply. Candidates of unknown provenance are scored as in-network.  
**Why it matters**: A bug that fails to set `in_network` on OON candidates would give them in-network scoring (no OON penalty) — they'd appear to rank higher than intended.
...
**Why it matters**: Which scorer is active depends on pipeline configuration. Running `WeightedScorer` means some action types (share_via_dm, share_via_copy_link, mute_author, quoted_click) are not included in the score.
